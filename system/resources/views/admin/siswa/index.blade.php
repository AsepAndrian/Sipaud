    ...
    <div class="nav_deg" style="margin: 10px;">
        <div
            ...
            <h1 style="margin: 0; padding: 0 10px; text-align: center; font-size: 20px; color: white;">
                Data siswa paud Kabupaten Ketapang
            </h1>
        </div>
    </div>
    ...
    <main class="main users chart-page" id="skip-target" style="padding: 20px; display: flex; justify-content: center;">
        <div class="table-container"
            ...
            <div
                ...
                <!-- Search Form -->
                <form id="search-form" action="{{ url('admin/siswa') }}" method="GET"
            ...
            <div style="overflow-x:auto;">
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                        @foreach ($list_siswas as $siswa)
                            <tr
                                style="background-color: #f8f8f8; transition: background-color 0.3s; border-bottom: 1px solid #ddd;">
                                <td style="padding: 12px;">{{ $loop->iteration }}</td>
                                <!-- Kolom Foto Profil -->
                                <td style="padding: 12px;">
                                    <img src="{{ url("public/$siswa->foto_siswa") }}"
                                        style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover;"
                                        onerror="this.src='https://bootdey.com/img/Content/avatar/avatar7.png';">
                                </td>
                                <td style="padding: 12px;">{{ $siswa->nama }}</td>
                                <td style="padding: 12px;">{{ $siswa->alamat }}</td>
                                <td style="padding: 12px;">{{ $siswa->jenis_kelamin }}</td>
                                <td style="padding: 12px; text-align: center;">
                                    ...
                                    <form action="{{ url('admin/siswa/' . $siswa->id) }}" method="POST"
                                        id="deleteForm{{ $siswa->id }}" style="display: inline;">
                                        @csrf
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="button" onclick="showConfirmModal({{ $siswa->id }})"
                                    ...
                                    <div id="confirmModal{{ $siswa->id }}"
                                        ...
                                        <div
                                            ...
                                            <button id="confirmDeleteBtn{{ $siswa->id }}"
                                            ...
                                            <button onclick="hideConfirmModal({{ $siswa->id }})"
                                    ...
                                    <script>
                                        function showConfirmModal(siswaId) {
                                            // Tampilkan modal
                                            document.getElementById('confirmModal' + siswaId).style.display = 'flex';

                                            // Set tindakan konfirmasi
                                            document.getElementById('confirmDeleteBtn' + siswaId).onclick = function() {
                                        ...
                                        function hideConfirmModal() {
                                            // Sembunyikan modal
                                            document.getElementById('confirmModal' + siswaId).style.display = 'none';
                                        }
                                    </script>
                                </td>
                            </tr>
                ...
                <div style="margin-top: 20px; display: flex; justify-content: center;">
                    <ul style="display: flex; list-style: none; padding: 0; gap: 5px;">
                        <!-- Tombol Sebelumnya -->
                        <li>
                            <a href="{{ $list_siswas->previousPageUrl() }}"
                                style="padding: 4px 8px; border-radius: 5px; {{ $list_siswas->onFirstPage() ? 'background-color: #ddd; color: #999; cursor: not-allowed;' : 'background-color: #4fb7e0; color: white;' }}">
                                &laquo; Sebelumnya
                            </a>
                        </li>

                        <!-- Nomor Halaman -->
                        @foreach ($list_siswas->getUrlRange(1, $list_siswas->lastPage()) as $page => $url)
                            <li>
                                <a href="{{ $url }}"
                                    style="padding: 4px 8px; border-radius: 5px; {{ $page == $list_siswas->currentPage() ? 'background-color: #4fb7e0; color: white;' : 'background-color: #e0e0e0; color: #333;' }}">
                                    {{ $page }}
                                </a>
                            </li>
                        @endforeach
                        ...
                        <!-- Tombol Berikutnya -->
                        <li>
                            <a href="{{ $list_siswas->nextPageUrl() }}"
                                style="padding: 4px 8px; border-radius: 5px; {{ $list_siswas->hasMorePages() ? 'background-color: #4fb7e0; color: white;' : 'background-color: #ddd; color: #999; cursor: not-allowed;' }}">
                                Berikutnya &raquo;
                            </a>
                        </li>
                    </ul>
